    ...
    for row in reversed(rows):
        lines.extend(
            [
                ...
                f"- campaign_id: {_v(row, 'campaign_id')}",
                f"- codec: {_v(row, 'codec')}",
                f"- status: {_v(row, 'status')}",
                f"- phase: {_v(row, 'phase')}",
                f"- stage: {_v(row, 'stage')}",
                f"- variant: {_v(row, 'variant')}",
                f"- track: {_v(row, 'track')}",
                f"- axis: {_v(row, 'axis')}",
                f"- family: {_v(row, 'family')}",
                f"- mode: {_v(row, 'mode')}",
                f"- config: {_v(row, 'config')}",
                f"- modal_path: {_v(row, 'modal_path')}",
                f"- brief_path: {_v(row, 'brief_path')}",
                f"- baseline_experiment_id: {_v(row, 'baseline_experiment_id')}",
